<x-admin-layout header="Penugasan Dosen">
    ...
    {{-- Filter live: dropdown auto-submit (GUIDELINE §6.6) --}}
    <form method="GET" class="mb-4">
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <x-select name="course_id" onchange="this.form.submit()">
                <option value="">Semua Mata Kuliah</option>
                @foreach ($courses as $course)
                    <option value="{{ $course->id }}" @selected(request('course_id') == $course->id)>{{ $course->code }} — {{ $course->name }}</option>
                @endforeach
            </x-select>
            <x-select name="lecturer_id" onchange="this.form.submit()">
            ...
            <x-select name="evaluation_period_id" onchange="this.form.submit()">
