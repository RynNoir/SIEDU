...
8. **[REVISI v1.4] Konsistensi paritas semester MK vs tipe periode**: `courses.semester` **ganjil** (1,3,5,7) hanya boleh diassign (`course_class_assignments`) ke `evaluation_periods` dengan `semester_type='ganjil'`; `courses.semester` **genap** (2,4,6,8) hanya ke `semester_type='genap'`. Kurikulum paket memang hanya menawarkan satu MK di semester ganjil ATAU genap sesuai `courses.semester`-nya — mengassign ke periode dengan tipe semester berbeda tidak masuk akal secara akademik dan sebelumnya tidak divalidasi (celah ditemukan & diperbaiki setelah rilis awal).
...
| v1.4 (2026-07-12) | **[BUGFIX]** Ditemukan celah validasi pada penugasan dosen (`course_class_assignments`): paritas semester MK (ganjil/genap) tidak pernah dicocokkan dengan `evaluation_periods.semester_type`, sehingga MK semester ganjil bisa ter-assign ke periode genap (dan sebaliknya) — kombinasi yang tidak masuk akal karena kurikulum paket. Ditambahkan validasi §7.8: `Course::semesterType()` (paritas semester) harus sama dengan `EvaluationPeriod::semester_type` saat membuat/mengubah penugasan. Tidak ada perubahan skema database. |